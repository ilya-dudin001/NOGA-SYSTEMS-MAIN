...
Личные данные видят и правят только **Owner** и **Правая рука** (право
`nogas:personal`). Администратор ногу видит, но паспортов, адреса и телефонов —
нет. Файлы лежат на сервере в `UPLOADS_DIR` (по умолчанию `backend/data/uploads`),
в базе только метаданные; удаление ноги удаляет и её файлы.
...
- **Ноги** — подтягиваются из справочника ног по привязке к городу: имя, тест/рабочая,
  кто добавил и когда. В форме города есть чек-лист ног: отмеченные работают в этом
  городе, снятые открепляются. В подписи видно, где нога сейчас — «без города» или
  «сейчас в Самаре».
...
(сами остаются в системе), и только потом удаляется город. Управляют Owner и Правая
рука, Администратор видит список без права правки.
...
`data/test_cities.db`, `data/test_noga_personal.db`) и не трогает рабочую.
