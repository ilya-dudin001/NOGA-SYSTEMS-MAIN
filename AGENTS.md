...
| Администратор | `admin` | Свой участок: заводит и правит **свои** города и ноги, читает чужие |
...
`cities:all` / `nogas:all` — право работать с **чужими** записями. Их нет у админа, поэтому
`*:manage` у него действует только на то, что он завёл сам (`created_by_id`). Проверки —
`can_manage_city` / `require_own_city` в `api/cities.py` и `can_manage` / `require_own`
в `api/nogas.py`; выдача фильтруется через `owner_id` в сервисах. Каждый город и каждая нога
в ответе несут `can_manage` — фронт по нему рисует иконки правки, а не гадает по роли.
...
`noga_id` — 403, а тёзка уже стоящей в городе чужой ноги — 409. Удаление разгруза
блокируется (409), пока он привязан хоть к одному городу.
...
Таббар зависит от роли (`NogaRoles.applyTabbar`, вызывается из `applySession`): у админа и
правой руки это «Панель / Ноги / + / Города / Профиль», у остальных прежние «Главная /
Операции / + / Поиск / Профиль». Лишние вкладки просто `hidden`, вся разметка лежит в
`index.html`. Плашка городов на дашборде открывает витрину (`mode: "working"`), вкладка
таббара — свой участок (`mode: "own"`); `NogaRoles.activateTab` подсвечивает нужную вкладку,
а если её нет — «Профиль».

Экран городов начинается с переключателя `#citiesModes` («Мои города» / «В работе»;
у ролей с `cities:all` первая вкладка называется «Все города»). Город в работе, к которому
не прикреплена ни одна нога, помечается баннером `.alert-banner` («!! ВНИМАНИЕ !! …») и
красным пульсирующим статусом `.status-pill--alert` — статус при этом не меняется, сервер
такие города не запрещает. Перед тем как поставить «В работе» городу без ног (переключателем
или из формы), фронт спрашивает подтверждение.
...
«Операции», «Поиск» и кнопка FAB ни к чему не привязаны. Отдельного экрана управления
городами у роли `noga` нет, а у админа нет доступа к созданию разгрузов (`razgruz:read`
без `razgruz:manage`) — в форме своего города он выбирает из уже заведённых.
