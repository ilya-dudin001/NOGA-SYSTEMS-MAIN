...
аватар с инициалами, имя и роль; Telegram ID, username, статус и даты спрятаны под
кнопкой-дропдауном `#profileDetailsToggle` (шеврон поворачивается через `is-open`,
`#profileRows` по умолчанию `hidden`). Каждый заход на экран сворачивает детали и
закрывает форму ника.

Кнопка «Изменить ник» (`#profileRenameBtn`) видна по `profile:rename`, то есть всем кроме
ноги. Она разворачивает `#profileRenameForm` прямо в карточке: поле, подсказка про
допустимые символы и «Отмена / Сохранить». Сохранение шлёт `NogaApi.updateMe`, ответ
кладётся в `NogaRoles.setUser` и `NogaDashboard.applyUser`, поэтому приветствие на
дашборде и инициалы в аватаре меняются сразу, без перезахода. Состав меню:
